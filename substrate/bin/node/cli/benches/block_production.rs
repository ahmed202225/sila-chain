...
use sila_runtime::{constants::currency::*, BalancesCall};
...
fn extrinsic_set_time(now: u64) -> OpaqueExtrinsic {
	let utx: sila_runtime::UncheckedExtrinsic = generic::UncheckedExtrinsic::new_bare(
		sila_runtime::RuntimeCall::Timestamp(pallet_timestamp::Call::set { now }),
	)
	.into();
	utx.into()
}
...
fn prepare_benchmark(client: &FullClient) -> (usize, Vec<OpaqueExtrinsic>) {
	...
	for nonce in 0.. {
		let extrinsic: OpaqueExtrinsic = create_extrinsic(
			client,
			src.clone(),
			BalancesCall::transfer_allow_death { dest: dst.clone(), value: 1 * SILA },
			Some(nonce),
		)
		.into();
