...
pub fn bloaty_code_unwrap() -> &'static [u8] {
	sila_runtime::WASM_BINARY_BLOATY.expect(
...
fn successful_execution_with_native_equivalent_code_gives_ok() {
	...
	t.insert(
		...
		AccountInfo::<<Runtime as frame_system::Config>::Nonce, _> {
			providers: 1,
			data: (111 * SILA, 0u128, 0u128, 1u128 << 127),
			..Default::default()
		}
		.encode(),
	);
	...
	t.execute_with(|| {
		assert_eq!(Balances::total_balance(&alice()), 42 * SILA - fees_after_refund);
		assert_eq!(Balances::total_balance(&bob()), 69 * SILA);
	});
}
...
fn full_native_block_import_works() {
	...
	t.execute_with(|| {
		assert_eq!(Balances::total_balance(&alice()), 42 * SILA - fees_after_refund);
		assert_eq!(Balances::total_balance(&bob()), 169 * SILA);
	...
	t.execute_with(|| {
		assert_eq!(
			...
			alice_last_known_balance - 10 * SILA - fees_after_refund,
		);
		assert_eq!(Balances::total_balance(&bob()), 179 * SILA - fees_after_refund);
		let events = vec![
			EventRecord {
...
fn full_wasm_block_import_works() {
	...
	t.execute_with(|| {
		...
		assert_eq!(Balances::total_balance(&bob()), 179 * SILA - 1 * fees_after_refund);
	});
}
...
fn successful_execution_gives_ok() {
	...
	assert!(r.is_ok());
	t.execute_with(|| {
		assert_eq!(Balances::total_balance(&alice()), 111 * SILA);
	});
