...
/// Helper: يحوّل عنوان SS58 إلى AccountId (ويفشل برسالة واضحة إن كان العنوان غير صالح)
#[inline]
fn acc(ss58: &str) -> AccountId {
    <AccountId as Ss58Codec>::from_string(ss58)
        .unwrap_or_else(|e| panic!("Invalid SS58 address `{}`: {e}", ss58))
}
/// Patch يضبط مفتاح sudo فقط (بدون أي أرصدة)
#[inline]
fn sudo_patch_only(root: AccountId) -> serde_json::Value {
    serde_json::json!({
        "sudo": { "key": root }
    })
}

/// Patch يضبط مفتاح sudo ويمنحه Endowment (استخدمه إن كان حساب sudo غير مذكور أصلًا في الـendowed)
#[inline]
fn sudo_with_endowment_patch(root: AccountId) -> serde_json::Value {
    serde_json::json!({
        "sudo": { "key": root.clone() },
        "balances": { "balances": vec![(root, ENDOWMENT)] }
    })
}


pub use sila_runtime::RuntimeGenesisConfig;
...
fn props() -> Properties {
    ...
    properties.insert("tokenSymbol".to_string(), "SILA".into());
    properties.insert("tokenDecimals".to_string(), 12.into());
    properties.insert("ss58Format".to_string(), 84.into()); 
    properties
}
...
pub(crate) mod tests {
	...
	use sila_runtime::genesis_config_presets::well_known_including_eth_accounts;
	use sc_service_test;
