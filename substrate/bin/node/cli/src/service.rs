...
/// Host functions required for sila runtime and Substrate node.
...
pub fn create_extrinsic(
	...
	function: impl Into<sila_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> sila_runtime::UncheckedExtrinsic {
	...
	let period = sila_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let tx_ext: sila_runtime::TxExtension =
		(
			frame_system::AuthorizeCall::<sila_runtime::Runtime>::new(),
			frame_system::CheckNonZeroSender::<sila_runtime::Runtime>::new(),
			frame_system::CheckSpecVersion::<sila_runtime::Runtime>::new(),
			frame_system::CheckTxVersion::<sila_runtime::Runtime>::new(),
			frame_system::CheckGenesis::<sila_runtime::Runtime>::new(),
			frame_system::CheckEra::<sila_runtime::Runtime>::from(generic::Era::mortal(
				period,
				best_block.saturated_into(),
			)),
			frame_system::CheckNonce::<sila_runtime::Runtime>::from(nonce),
			frame_system::CheckWeight::<sila_runtime::Runtime>::new(),
			pallet_skip_feeless_payment::SkipCheckIfFeeless::from(
				pallet_asset_conversion_tx_payment::ChargeAssetTxPayment::<
					sila_runtime::Runtime,
				>::from(tip, None),
			),
			frame_metadata_hash_extension::CheckMetadataHash::new(false),
			frame_system::WeightReclaim::<sila_runtime::Runtime>::new(),
		);

	let raw_payload = sila_runtime::SignedPayload::from_raw(
		function.clone(),
		tx_ext.clone(),
		(
			(),
			(),
			sila_runtime::VERSION.spec_version,
			sila_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	generic::UncheckedExtrinsic::new_signed(
		...
		sila_runtime::Signature::Sr25519(signature),
		tx_ext,
	)
	.into()
}
...
mod tests {
	...
	use codec::Encode;
	use sila_runtime::{
		constants::{currency::uSILA, time::SLOT_DURATION},
	...
	fn test_sync() {
		...
		sc_service_test::sync(
			...
			|service, _| {
				...
				index += 1;
				let utx: sila_runtime::UncheckedExtrinsic =
					generic::UncheckedExtrinsic::new_signed(
						function,
						from.into(),
						signature.into(),
